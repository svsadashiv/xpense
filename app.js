...
function renderLoans() {
  ...
  activeLoans.forEach(l => {
    ...
    // Find the EMI whose due date falls in the current month
    const thisMonthEMI = schedule.find(e => {
      const d = new Date(e.date);
      return d.getMonth() + 1 === thisMonth && d.getFullYear() === thisYear;
    });

    if (!thisMonthEMI) return; // no EMI due this month for this loan

    const gst      = gstRate > 0 ? (thisMonthEMI.interest * gstRate / 100) : 0;
    const emiTotal = emi + gst;

    if (paidNums.includes(thisMonthEMI.n)) {
      paidThisMonth += emiTotal;
      paidCount++;
    } else {
      pendingThisMonth += emiTotal;
      pendingCount++;
      pendingLoans.push({ name: l.name, amount: emiTotal, date: thisMonthEMI.date });
    }
  });
  ...
  const monthlySummary = hasActivity ? `
  <div class="card" style="margin-bottom:20px;border-left:4px solid var(--blue)">
    <div style="font-size:13px;font-weight:700;color:var(--text3);text-transform:uppercase;letter-spacing:.5px;margin-bottom:14px">📅 ${monthLabel} — EMI Summary</div>
    <div style="display:grid;grid-template-columns:1fr 1fr;gap:12px;margin-bottom:${pendingCount > 0 ? '14px' : '0'}">
      <div style="background:#D1FAE5;border-radius:10px;padding:12px">
        <div style="font-size:12px;color:#065F46;margin-bottom:4px">✅ Paid (${paidCount} EMI${paidCount !== 1 ? 's' : ''})</div>
        <div style="font-size:20px;font-weight:700;color:#065F46">${DB.fmtINR(paidThisMonth)}</div>
      </div>
      <div style="background:${pendingCount > 0 ? '#FEE4E2' : '#F3F4F6'};border-radius:10px;padding:12px">
        <div style="font-size:12px;color:${pendingCount > 0 ? 'var(--red)' : 'var(--text3)'};margin-bottom:4px">${pendingCount > 0 ? '⏳' : '🎉'} Pending (${pendingCount} EMI${pendingCount !== 1 ? 's' : ''})</div>
        <div style="font-size:20px;font-weight:700;color:${pendingCount > 0 ? 'var(--red)' : 'var(--text3)'}">${DB.fmtINR(pendingThisMonth)}</div>
      </div>
    </div>
    ${pendingCount > 0 ? `
    <div style="border-top:1px solid var(--border);padding-top:10px">
      <div style="font-size:12px;color:var(--text3);margin-bottom:8px;font-weight:600">Due this month:</div>
      ${pendingLoans.map(p => `
      <div style="display:flex;justify-content:space-between;align-items:center;padding:6px 0;border-bottom:1px solid var(--border);font-size:13px">
        <span style="color:var(--text1);font-weight:500">${escapeHTML(p.name)}</span>
        <span style="color:var(--red);font-weight:700">${DB.fmtINR(p.amount)}</span>
      </div>`).join('')}
    </div>` : ''}
  </div>` : '';
...
function buildLoanMonthlySummary(activeLoans, txns, allLoans) {
  ...
  loansToScan.forEach(l => {
    ...
    if (!thisMonthEMI) return;
    const gst      = gstRate > 0 ? (thisMonthEMI.interest * gstRate / 100) : 0;
    const emiTotal = emi + gst;
    if (paidNums.includes(thisMonthEMI.n)) {
      paidThisMonth += emiTotal; paidCount++;
    } else {
      pendingThisMonth += emiTotal; pendingCount++;
      pendingLoans.push({ name: l.name, amount: emiTotal });
    }
  });

  if (paidCount === 0 && pendingCount === 0) return '';
  const monthLabel = now.toLocaleDateString('en-IN', { month: 'long', year: 'numeric' });
  return `
  <div class="card" style="margin-bottom:20px;border-left:4px solid var(--blue)">
    <div style="font-size:13px;font-weight:700;color:var(--text3);text-transform:uppercase;letter-spacing:.5px;margin-bottom:14px">📅 ${monthLabel} — EMI Summary</div>
...
// Surgically refreshes the loans KPI strip + monthly summary + active loan cards
// without re-rendering the full page (preserves scroll position).
function refreshLoansUI() {
  const kpiWrap    = document.getElementById('loans-kpi-wrap');
  const activeWrap = document.getElementById('loans-active-wrap');
  if (!kpiWrap || !activeWrap) { renderTab('loans'); return; }
  ...
  const completedBtn = document.getElementById('completed-loans-btn');
  if (completedBtn) {
    completedBtn.querySelector('span:first-child').textContent = `✅ Completed Loans (${completedLoans.length})`;
    const completedList = document.getElementById('completed-loans-list');
    if (completedList) completedList.innerHTML = completedLoans.map(l => loanCardHTML(l, txns)).join('');
  }
}
...
function openAddTransaction(editId = null) {
  ...
  const ccOpts = cards.length === 0
    ...
    : '<select class="form-select" id="txn-card">' +
        cards.map((c,i) => '<option value="'+c.id+'"'+(edit && edit.payment === 'credit-card' && edit.creditCardId===c.id ? ' selected' : (!edit && i===0 ? ' selected':''))+'>'+c.name+' ('+c.bank+' \u00b7\u00b7\u00b7\u00b7 '+c.last4+') — Avail: '+DB.fmtINR(c.limit-c.dueBalance)+'</option>').join('') +
      '</select>';
  const paydownOpts = cards.length === 0
    ? '<p style="color:var(--orange);font-size:13px">No cards added yet.</p>'
    : '<select class="form-select" id="txn-paydown-card">' +
        '<option value="">— Not paying toward a card —</option>' +
        cards.map(c => '<option value="'+c.id+'"'+(paydownSelected===c.id?' selected':'')+'>'+c.name+' ('+c.bank+' \u00b7\u00b7\u00b7\u00b7 '+c.last4+')</option>').join('') +
      '</select>';
